...
import { makeCharKey, makeNumpadKey, makeArrowKey } from './keycode';
import { Plane, Weapon } from './core';

import { promiseAll, gameoverImg } from './asset';

import type { GlobalSetting, Init } from './init';
import { globalSetting, defaultInitPlayer } from './init';

export class Gameover {
  constructor(
    public readonly winPlane: Plane,
    public readonly diedPlane: Plane[]
  ) {}
}
...
export class Controller {
  private planes: Plane[] = []; // 正在进行管理的飞机，在进行中加进来也是有效的
  private diedPlane: Plane[] = [];
  private overgameCbk: GameoverCbk[] = [];
  ...
  appendGameoverCbk(fn: GameoverCbk) {
    this.overgameCbk.push(fn);
  }

  async start(init: Init = defaultInitPlayer) {
    this.reset();
    ...
    (function run() {
      ...
      try {
        ...
        if (e instanceof Gameover) {
          overgameCbk().forEach(cbk => cbk(e as Gameover));
          return;
        }
